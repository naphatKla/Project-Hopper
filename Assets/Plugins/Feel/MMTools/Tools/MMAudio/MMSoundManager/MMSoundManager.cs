...
namespace MoreMountains.Tools
{
		...
		MMEventListener<MMSoundManagerTrackFadeEvent>
	{
		...
		{
			_sounds.RemoveAll(x => !x.Source); //Added Later, not sure if it will interfere with existing system
		...
		public virtual void FadeTrack(MMSoundManagerTracks track, float duration, float initialVolume = 0f, float finalVolume = 1f, MMTweenType tweenType = null, bool pauseAfterFade = false)
		{
			//This is a hack
			if (_fadeTrackCoroutines.ContainsKey(track) && _fadeTrackCoroutines[track] != null) 
			{
				StopCoroutine(_fadeTrackCoroutines[track]);
				_fadeTrackCoroutines[track] = null;
			}
			Coroutine coroutine = StartCoroutine(FadeTrackCoroutine(track, duration, initialVolume, finalVolume, tweenType, pauseAfterFade));
		...
		/// <param name="finalVolume"></param>
		/// <param name="tweenType"></param>
		public virtual void FadeSound(AudioSource source, float duration, float initialVolume, float finalVolume, MMTweenType tweenType, bool freeAfterFade = false, bool stopAfterFade = true)
		{
			Coroutine coroutine = StartCoroutine(FadeCoroutine(source, duration, initialVolume, finalVolume, tweenType, freeAfterFade, stopAfterFade));
		...
		/// <param name="tweenType"></param>
		/// <returns></returns>
		protected virtual IEnumerator FadeTrackCoroutine(MMSoundManagerTracks track, float duration, float initialVolume, float finalVolume, MMTweenType tweenType, bool pauseAfterFade = false)
		{
		...
		/// <param name="tweenType"></param>
		/// <returns></returns>
		protected virtual IEnumerator FadeCoroutine(AudioSource source, float duration, float initialVolume, float finalVolume, MMTweenType tweenType, bool freeAfterFade = true, bool stopAfterFade = true)
		{
			float startedAt = Time.unscaledTime;
			if (tweenType == null)
			{
